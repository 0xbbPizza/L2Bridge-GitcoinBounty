...
contract DestChildContract {
    ...
    function getFork(uint256 forkKeyNum) external view returns (Data.HashOnionFork memory fork) {
        fork = hashOnionForks[forkKeyNum];
    }

    function getForkKeyNum(bytes32 key , uint256 index) external view returns (uint256 keyNum){
        keyNum = forkKeysMap[key][index];
    }

    function setForKeyNum(bytes32 key , uint256 index , uint256 keyNum) external onlyRouter {
        forkKeysMap[key][index] = keyNum;
    }

    function setFork(bytes32 key , uint256 index, Data.HashOnionFork calldata fork) external onlyRouter {
        forkKeysMap[key][index] = ++forkKeyID;
        hashOnionForks[forkKeyID] = fork;
    }

    function setForkWithForkKey(uint256 forkKeyNum, Data.HashOnionFork calldata fork) external onlyRouter {
    ...
    function getIsRepondOnion(bytes32 onion) external view returns (bool state) {
