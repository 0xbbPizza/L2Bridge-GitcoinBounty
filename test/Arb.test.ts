...
describe("Arb", function () {
  ...
  it("Dock_Arb", async function () {
    ...
    const messageInfo = [
      RinkebyArbitrumChainId,
      "hello world through Bridge XXXXXXXX ",
    ];
    ...
    const l1ToL2Message = (
      await l1TxReceipt.getL1ToL2Messages(
        RinkebyArbitrumProvider /** <-- connected ethers-js Wallet */
      )
    )[0];

    const status = await l1ToL2Message.waitForStatus();
    console.log("status", status);
    // console.log("getDock1Balance", await dockL1_AR.getDockBalance());
    ...
    // console.log("getDockBalance1", await dockL1_AR.getDockBalance());
