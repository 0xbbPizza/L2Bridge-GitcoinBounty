...
library Fork {
    ...
    function clear(mapping(uint256 => Fork.Info) storage self, uint256 forkId) internal {
        delete self[forkId];
    }

    function create(mapping(uint256 => Fork.Info) storage self, uint256 forkId) internal view pure returns (Info memory forkInfo){

    }

    function updata(mapping(uint256 => Fork.Info) storage self, uint256 forkId) internal {

    }

    function creatInfoKey(uint256 chainId, bytes32 hashOnion, uint256 index) internal view pure returns (bytes32) {
