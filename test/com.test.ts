...
describe("source", function () {
  ...
  it("only zbond on dest", async function () {
    ...
    await dest.setHashOnion(
      "0x003d8a08f9ede42ea05521791c51a4fda7c79781c03131deef9f90cfd5741aaa"
    );
    expect(await dest.sourceHashOnion()).to.equal(
      "0x003d8a08f9ede42ea05521791c51a4fda7c79781c03131deef9f90cfd5741aaa"
    );
    expect(await dest.onWorkHashOnion()).to.equal(hashOnion);
    ...
    expect(await fakeToken.balanceOf(accounts[0].getAddress())).to.equal(
      bonderAmount - sourceAmount
    );
